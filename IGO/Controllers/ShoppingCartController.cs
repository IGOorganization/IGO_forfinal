    ...
    public class ShoppingCartController : Controller
    {
        ...
        public JsonResult Delete(int id)
        {
            try
            {
                ...
                if (shoppingCartItem.FCouponId > 0)
                {
                    var coupon = (_dbIgo.TShoppingCarts.Where(c => c.FCouponId == shoppingCartItem.FCouponId && c.FTempOrder == shoppingCartItem.FTempOrder && c.FCustomerId == HttpContext.Session.GetInt32(CDictionary.SK_LOGINED_USER))).ToList();
                    foreach (var data in coupon)
                    {
                        _dbIgo.TShoppingCarts.Remove(data);
                    }
                }
                else
                {
        ...
        public IActionResult Pay()
        {
            ...
            foreach (TShoppingCart data in _dbIgo.TShoppingCarts.Where(c => IDs.Contains(c.FShoppingCartId)).ToList())
            {
                ...
                if (data.FCouponId > 0)
                {
                    var priceList = (_dbIgo.TShoppingCarts.Where(c => c.FCouponId == data.FCouponId &&c.FTempOrder == data.FTempOrder && c.FCustomerId == HttpContext.Session.GetInt32(CDictionary.SK_LOGINED_USER))).ToList();
                    foreach (var singlePrice in priceList)
                    {
                        totalPrice += (int)singlePrice.FTotalPrice;
                    }
                    shoppingCartViewModel.shoppingCart = data;
                    shoppingCartViewModel.FTotalPrice = totalPrice;
                    lists.Add(shoppingCartViewModel);
